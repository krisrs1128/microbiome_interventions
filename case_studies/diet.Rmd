...
fits <- list(mdsine = list(), fido = list())
ts_preds <- list(mdsine = list(), fido = list())
fmla <- ~ 1 + time + Plant + Animal + subject
fits[["fido"]][["in-sample"]] <- fido(ts, 2, 2, fmla)
ts_preds[["fido"]][["in-sample"]] <- fido_predict(fits[["fido"]][["in-sample"]], ts, fmla)
ts_missing <- subset_values(ts, 1:8)
ts_preds[["fido"]][["in-sample"]] <- fido_predict(fits[["fido"]][["in-sample"]], ts_missing, fmla)
```
...
```{r}
fits[["fido"]][["out-of-sample"]] <- fido(ts[c(1:5, 11:15)], 2, 2, fmla)
ts_preds[["fido"]][["out-of-sample"]] <- fido_predict(fits[["fido"]][["out-of-sample"]], ts_missing[c(6:10, 16:20)], fmla)
```


```{r, fig.width = 10, fig.height = 6}
subject_data(ts_preds[["fido"]][["in-sample"]]) <- subject_data(ts_preds[["fido"]][["in-sample"]]) |>
  mutate(subject = as.factor(as.character(subject)))
subject_data(ts_preds[["fido"]][["out-of-sample"]]) <- subject_data(ts_preds[["fido"]][["out-of-sample"]]) |>
  mutate(subject = as.factor(as.character(subject)))
reshaped_preds <- map_dfr(ts_preds[["fido"]], ~ reshape_preds(ts, .), .id = "generalization") |>
  filter(h > 0, h < 6)

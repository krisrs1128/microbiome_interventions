...
n_subject <- n_distinct(samples$subject)
subject_data <- tibble(
  subject = unique(samples$subject),
  x = rnorm(n_subject)
)

ts_inter <- ts_from_dfs(reads, interventions, samples, subject_data)
...
```{r}
taxonomy <- read_tsv(data_dir / "taxonomy.tsv")
...
metrics <- list()
metrics[["gbm"]] <- cross_validate(ts, tr_fun("gbm"))
```
...
taxonomy <- read_csv(data_dir / "taxaTable.csv") %>%
  ...
  filter(name %in% rownames(values(ts[[1]]))) %>%
  rename(kingdom = superkingdom)

taxa_names <- rownames(values(ts[[1]]))
new_ix <- !(taxa_names %in% taxonomy$name)
taxonomy <- taxonomy %>%
  bind_rows(data.frame(name = taxa_names[new_ix]))

hyper <- list(taxonomy = taxonomy3)
metrics[["mdsine"]] <- cross_validate(ts, tr_fun("mdsine", hyper))
```

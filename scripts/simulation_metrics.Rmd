...
result <- cross_validate(ts, tr_fun(method, hyper), K = 2)
metrics <- result$metrics
```

```{r}
values_mat <- do.call(rbind, map(ts@series, ~ values(.)))
reference_ae <- mean(abs(values_mat - mean(values_mat)))
ggplot(metrics) +
  geom_boxplot(aes(lag, mae)) +
  geom_hline(yintercept = reference_ae)
```
...
effects <- pd_splits(ts, w$w0, w$w1, 2, method = method, hyper = hyper)

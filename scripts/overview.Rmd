...
composition_plot <- function(ts_df, width = 1, ymax = 1, n_taxa = 10, ylab = "Intervention") {
  if (n_taxa == 10) {
...
p1 <- ts_df[[1]] |>
  ...
  composition_plot(ymax = 0.42, ylab = "Animal Diet")

p2 <- ts_df[[2]] |>
  filter(subject %in% c("S10004")) |>
  rename("intervention" = "birth") |>
  composition_plot(width = 14, ymax = 0.55, ylab = "Birth Period")

p3 <- ts_df[[3]] |>
  filter(subject %in% c("T1")) |>
  rename("intervention" = "pH") |>
  composition_plot(width = 1, ymax = 0.26, ylab = "pH")
```

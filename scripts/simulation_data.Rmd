...
nonnull_taxa <- list(seq(n_taxa * prop_nonnull))
```
...
for (i in seq(2, n_lag)) {
  e <- seq_len(n_taxa) %in% nonnull_taxa[[i - 1]]
  nonnull_taxa[[i]] <- which((A[[i - 1]] %*% ifelse(e, 1, 0)) != 0)
}
```
...
values_df <- data.frame(reads) |>
  ...
  filter(taxon %in% str_c("tax", c(1:3, 40, 38, 9))) |>
  left_join(metadata) |>
  group_by(taxon) |>
...
ggplot(values_df, aes(time, reorder(subject, value))) +
  geom_tile(aes(fill = value, col = as.factor(P1)), size = .1) +

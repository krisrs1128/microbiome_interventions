...
computation |>
  ...
  ggplot() +
  geom_boxplot(
    aes(normalization, time_diff, col = interaction(method, P), fill = interaction(method, P)), 
    alpha = 0.6,
...
for (norm_n in c("none", "DESeq2", "DESeq2-asinh")) {
  cur_metrics <- metrics |>
    mutate(
      n_taxa = factor(glue("{n_taxa} taxa"), levels = c("50 taxa", "100 taxa", "200 taxa")),
  ...
  metrics_range <- cur_metrics |>
    filter(method != "mdsine") |>
    summarise(
      min = 0.5 * quantile(mae, 0.25, na.rm = T),
      max = 1.5 * quantile(mae, 0.75, na.rm = T)
    )

  ggplot(cur_metrics) +
    geom_boxplot(
      aes(n_taxa, mae, fill = interaction(factor(method), P), col = interaction(factor(method), P)), 
      alpha = 0.5,
      position = position_dodge2(preserve = "single")
      ) +
    scale_fill_brewer(palette = "Set1") +
  ...
  ggsave(glue("forecasting-sim-{norm_n}.png"), height = 6, width = 12)
}
...
testing_results |>
  ...
  facet_grid(n_taxa + normalization ~ prop_nonnull + method) +

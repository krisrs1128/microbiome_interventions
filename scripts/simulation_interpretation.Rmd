...
computation |>
  ...
  ggplot() +
  geom_boxplot(
    aes(reorder(normalization, time_diff), time_diff, col = interaction(method, hyperparameter), fill = interaction(method, hyperparameter)), 
    alpha = 0.6,
...
for (norm_n in c("none", "DESeq2", "DESeq2-asinh")) {
  ...
  metrics_range <- cur_metrics |>
    ...
    summarise(
      min = median(mae, na.rm = T) - 3 * IQR(mae, na.rm = T),
      max = max(0, median(mae, na.rm = T) + 3 * IQR(mae, na.rm = T))
    )
  ...
  p <- ggplot(cur_metrics) +
    ...
    scale_x_discrete(labels = ~ gsub("\\.λ", ",\tλ", .x)) +
    scale_fill_brewer(palette = "Set1") +
    scale_color_brewer(palette = "Set1") +
    facet_grid(phylo_alpha + baseline_lambda ~ n_taxa + prop_nonnull) +
    labs(
      x = "Number of Taxa", 
  ...
  ggsave(glue("forecasting-sim-no-truncation{norm_n}.png"), height = 8, width = 14)
  
  p <- p + scale_y_continuous(limits = c(metrics_range$min, metrics_range$max))
  ggsave(glue("forecasting-sim-{norm_n}.png"), height = 8, width = 14)
}
```
...
testing_results |>
  filter(type == "marginal", n_taxa == 200) |>
  mutate(
    prop_nonnull = glue("{100 * prop_nonnull}% nonnull"),
    signal_B = glue("b = {signal_B}")
  ) |>
  ...
  geom_point(aes(fdp, power, shape = signal_B), size = 3.5) +
  ...
  facet_grid(normalization + phylo_alpha ~ prop_nonnull + method) +
...
ggsave("inference-sim-error.png", height = 6.5, width = 11)

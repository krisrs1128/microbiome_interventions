...
for (norm_n in c("none", "DESeq2", "DESeq2-asinh")) {
  ...
  p <- ggplot(cur_metrics) +
    geom_boxplot(
      aes(signal_B, mae, fill = interaction(factor(method), hyperparameter, sep = ""), col = interaction(factor(method), hyperparameter, sep = "")), 
      alpha = 0.5,
      position = position_dodge2(preserve = "single")
      ) +
    scale_fill_brewer(palette = "Set1") +
    scale_color_brewer(palette = "Set1") +
    facet_wrap(n_taxa ~ prop_nonnull, ncol = 3) +
    labs(
      x = "Number of Taxa", 
...
testing_results |>
  ...
  facet_grid(normalization + n_taxa ~ prop_nonnull + method) +

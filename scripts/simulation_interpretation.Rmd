...
computation |>
  left_join(metadata) |>
  ggplot() +
  geom_boxplot(aes(normalization, as.numeric(time_diff), col = factor(method), fill = factor(method)), alpha = 0.6) +
...
testing_results |>
  ...
  facet_grid(normalization ~ method + prop_nonnull) +
...
ggsave("~/Desktop/laboratory/talks/2023/20230414/figures/inference-sim-error.png", height = 5, width = 10)

summary_table <- testing_results |>
  filter(type == "marginal") |>
  group_by(method, normalization, lag, prop_nonnull, signal_B) |>
  summarise(FDP = round(mean(fdp), 3))

write_csv(summary_table, "~/Desktop/laboratory/talks/2023/20230414/figures/summary_table_fdp.csv")
```
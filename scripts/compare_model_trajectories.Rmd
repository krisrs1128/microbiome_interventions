...
cvs[["mbtransfer"]] <- cross_validate(ts, \(x) mbtransfer(x, 4, 4), K = 4, seeds = 1:4)
predict <- fido2::predict
cvs[["fido"]] <- cross_validate(ts, \(x) fido(x, sigma = .5, rho = .5), K = 4, seeds = 1:4)
...
ggsave("~/Desktop/writing/202305/mbtransfer/figure/differenced_data.png", height = 4.5, width = 12)
```
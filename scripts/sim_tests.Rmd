...
```{r}
pdp_generator <- function(ts, lags) {
...
```{r}
pdp_effects <- function(ts, w0, w1, lags = NULL, ...) {
  N <- length(ts)
  
  split_ix <- sample(N, 0.5 * N)
  fits <- list(
    train(ts[split_ix], ...)@parameters,
    train(ts[-split_ix], ...)@parameters
  )
  ...
  effects <- matrix(nrow = length(fits[[1]]), ncol = 2)
  pdp_fun <- pdp_generator(ts, lags)
  for (s in seq_along(fits)) {
    effects[, s] <- pdp_fun(fits[[s]], w1) - pdp_fun(fits[[s]], w0)
  }
  
  effects
}

pdp_mirror <- function(ts, w0, w1, lags = NULL, ...) {
  effects <- pdp_effects(ts, w0, w1, lags, ...)
  sgn <- sign(effects[, 1] * effects[, 2])
  magnitude <- rowMeans(sgn)
  sgn * magnitude
}
...
m <- pdp_mirror(ts, w0, w1, method = "gbm")

...
```{r}
matnorm <- function(n, m, mu = 0, sd = 1) {
  matrix(rnorm(n * m, mu, sd), n, m)
}
...
```{r}
temporal_mirror_ <- function(splits) {
  tau <- map(splits, ~ .$y1 - .$y0)
  int_tau <- map(tau, rowSums)
  sgn <- sign(int_tau[[1]] * int_tau[[2]])
  
  int_abs_tau <- map(tau, ~ rowMeans(abs(.)))
  magnitude <- int_abs_tau[[1]] + int_abs_tau[[2]]
...
temporal_mirror2 <- function(splits) {
...
m <- temporal_mirror2(splits)
print(m[1:n_nonnull])
hist(m, 100)
```
What if we have a different pairs / split for each person?

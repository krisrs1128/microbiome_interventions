...
B <- low_rank_step(n_taxa, n_latent, n_perturb, n_lag, sigma = 1)
...
Let's create some example counterfactual series from which to generate mirror
statistics. We will want to ensure that the mirrors are in fact symmetric in the
null cases.
...
n_nonnull <- 5
splits <- list(
  list(y0 = matrix(rnorm(n_time * n_taxa), n_taxa, n_time)),
  list(y0 = matrix(rnorm(n_time * n_taxa), n_taxa, n_time))
)

y11 <- splits[[1]]$y0 + rnorm(length(splits[[1]]$y0), 0, 1)
signal <- 10
y11[seq_len(n_nonnull), 1:5] <- y11[seq_len(n_nonnull), 1:5] + rnorm(n_nonnull * 5, signal, .5)

y21 <- splits[[2]]$y0 + rnorm(length(splits[[2]]$y0), 0, 1)
y21[seq_len(n_nonnull), 1:5] <- y11[seq_len(n_nonnull), 1:5] + rnorm(n_nonnull * 5, signal, 0.5)
...
#    y1
splits <- list(
  replicate(n_subject, list(y0 = matnorm(n_taxa, n_time)), simplify = F),
  replicate(n_subject, list(y0 = matnorm(n_taxa, n_time)), simplify = F)
)

for (s in seq_along(splits)) {
  for (i in seq_along(splits[[s]])) {
    splits[[s]][[i]]$y1 <- splits[[s]][[i]]$y0 + rnorm(length(splits[[s]][[i]]$y0), 0, 1)
    splits[[s]][[i]]$y1[seq_len(n_nonnull), 1:5] <- splits[[s]][[i]]$y1[seq_len(n_nonnull), 1:5] + rnorm(n_nonnull * 5, signal, 1)
  }
  
}

plot(splits[[1]][[1]]$y1[1, ], col = "red")
points(splits[[1]][[1]]$y0[1, ])
```
...
m <- temporal_mirror(splits)
print(m[1:(2 * n_nonnull)])
hist(m, 100)
```
...
synthetic_control <- function(ts, n_lag = 4, ...) {
  fit <- train(ts, ...)
  controls <- ts
  print("training")
  ...
  for (i in seq_along(ts)) {
    ...
    first_inter <- max(n_lag, min(which(colSums(w) != 0)))
    v <- values(controls[[i]])
...
split_inter <- list(
  list(
    ts = ts[split_ix],
    control = synthetic_control(ts[split_ix], method = "gbm")
  ),
  list(
    ts = ts[-split_ix],
    control = synthetic_control(ts[-split_ix], method = "gbm")
...
splits <- list()
for (s in seq_along(split_inter)) {
  split_s <- list()
  for (i in seq_along(split_inter[[s]]$ts)) {
    split_s[[i]] <- list(
      y1 = values(split_inter[[s]]$ts[[i]]),
      y0 = values(split_inter[[s]]$control[[i]])
    )
  }
  splits[[s]] <-  split_s
}
...
```{r}
# a few of the taxa with actual differences
for (i in nonnull_taxa) {
  plot(splits[[1]][[1]]$y0[i, ])
  points(splits[[1]][[1]]$y1[i, ], col = "red")
}
```


```{r}
# and a few with no åctual difference
for (i in seq(setdiff(1:5, nonnull_taxa))) {
  plot(splits[[1]][[1]]$y0[i, ])
  points(splits[[1]][[1]]$y1[i, ], col = "red")
}
```

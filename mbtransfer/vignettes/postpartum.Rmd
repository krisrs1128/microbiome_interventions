...
```
```{r}
reads <- read_csv("~/Downloads/postpartum/reads.csv") |>
  column_to_rownames("sample")
metadata <- read_csv("~/Downloads/postpartum/metadata.csv")
samples <- read_csv("~/Downloads/postpartum/samples.csv")
subject_data <- read_csv("~/Downloads/postpartum/subject_data.csv")
interventions <- read_csv("~/Downloads/postpartum/interventions.csv") |>
  column_to_rownames("sample")
```
...
ts_preds[["out_of_sample"]] <- predict(fit, ts_missing[c(6:10, 16:20)])
```
...
subject_errors <- map_dfr(ts_preds[["in-sample"]] - ts, ~ tibble(mse = mean(abs(values(.)))), .id = "subject")
